...
export generalizedItermultiDimZdeepest,
	@iterateLinearlyCheckAll, @iterateLinearlyMultipleBlocks, @iterateLinearlyWithStart, @iterateLinearly, @exOnWarp, @exOnWarpIfBool, @iter3d, @iter3dAdditionalxyzActsAndZcheck, @iter3dAdditionalxyzActs, @iter3dAdditionalzActs, @iter3dWithVal
...
"""
macro iter3dAdditionalxyzActsAndZcheck(arrDims, loopXdim, loopYdim, loopZdim, zCheck, ex, additionalActionAfterX, additionalActionAfterY, additionalActionAfterZ)
	mainExp = generalizedItermultiDim(; arrDims = arrDims, loopXdim = loopXdim, loopYdim = loopYdim, loopZdim = loopZdim, zCheck = zCheck, ex = ex, additionalActionAfterX = additionalActionAfterX, additionalActionAfterY = additionalActionAfterY,
...
macro iter3dAdditionalzActs(arrDims, loopXdim, loopYdim, loopZdim, ex, additionalActionAfterZ)
	...
	mainExp = generalizedItermultiDim(; arrDims = arrDims, loopXdim = loopXdim, loopYdim = loopYdim, loopZdim = loopZdim, additionalActionAfterZ = additionalActionAfterZ, ex = ex)
...
function generalizedItermultiDim(; #we keep all as keyword arguments
	arrDims = (UInt32(1), UInt32(1), UInt32(1)), xname = :x, yname = :y, zname = :z, loopIterNameX = :xdim, loopIterNameY = :ydim, loopIterNameZ = :zdim, loopXdim = 1, loopYdim = 1, loopZdim = 1, zOffset = :(zdim * gridDim().x), zAdd = :(blockIdxX()),
	yOffset = :(ydim * blockDimY()), yAdd = :(threadIdxY()), xOffset = :(xdim * blockDimX()), xAdd = :(threadIdxX()), xCheck = :($xname <= $arrDims[1]), yCheck = :($yname <= $arrDims[2]), zCheck = :($zname <= $arrDims[3]), additionalActionAfterZ = :(),
	additionalActionAfterY = :(), additionalActionAfterX = :(), additionalActionBeforeZ = :(), additionalActionBeforeY = :(), additionalActionBeforeX = :()
	, is3d = true, ex, isFullBoundaryCheckX = false, isFullBoundaryCheckY = false, isFullBoundaryCheckZ = true, nobundaryCheckX = false, nobundaryCheckY = false, nobundaryCheckZ = false, isVal = false, arrMain = :(), loopStartX = 0, loopStartY = 0,
	loopStartZ = 0,)
