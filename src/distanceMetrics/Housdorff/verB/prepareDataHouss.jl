...
"""
function getBoolCubeKernel(goldBoolGPU3d, segmBoolGPU3d, reducedGoldA, reducedSegmA, reducedGoldB, reducedSegmB, loopNumbYdim::UInt16, xdim::UInt16, loopNumbXdim::UInt16, numberToLooFor::T, fn::CuDeviceVector{UInt32, 1}, fp::CuDeviceVector{UInt32, 1},
	minxRes::CuDeviceVector{UInt32, 1}, maxxRes::CuDeviceVector{UInt32, 1}, minyRes::CuDeviceVector{UInt32, 1}, maxyRes::CuDeviceVector{UInt32, 1}, minzRes::CuDeviceVector{UInt32, 1}, maxzRes::CuDeviceVector{UInt32, 1}, dataBdim,
) where T
	...
	@iter3dOuter(xname, yName, zName, loopXMeta, loopYMeta, loopZmeta,
		begin
			...
			@iter3dInMeta(xOuter * dataBdim[1], yOuter * dataBdim[2], zzOuter * dataBdim[3], true, zdim, inBlockLoopX, inBlockLoopY, inBlockLoopZ, begin
			...
			@ifXY 1 1 if (isAnyPositive[1])
				minX[1] = min(minX[1], xOuter)
			end
			@ifXY 2 1 if (isAnyPositive[1])
				maxX[1] = max(maxX[1], xOuter)
			end
			@ifXY 3 1 if (isAnyPositive[1])
				minY[1] = min(minY[1], yOuter)
			end
			@ifXY 4 1 if (isAnyPositive[1])
				maxY[1] = max(maxY[1], yOuter)
			end
			@ifXY 5 1 if (isAnyPositive[1])
				minZ[1] = min(minZ[1], zOuter)
			end
			@ifXY 6 1 if (isAnyPositive[1])
				maxZ[1] = max(maxZ[1], zOuter)
			end
			@ifXY 7 1 if (isAnyPositive[1])
				isAnyPositive[1] = false
			end #reset 


			@ifXY 1 2 if (isAnyPositive[1])
				setMetaLeftFP(metaData, leftFP[1])
			end
			@ifXY 2 2 if (isAnyPositive[1])
				setMetaLeftFN(metaData, leftFN[1])
			end

			@ifXY 3 2 if (isAnyPositive[1])
				setMetaRightFP(metaData, rightFP[1])
			end
			@ifXY 4 2 if (isAnyPositive[1])
				setMetaRightFN(metaData, rightFN[1])
			end

			@ifXY 5 2 if (isAnyPositive[1])
				setMetaPosteriorFP(metaData, posteriorFP[1])
			end
			@ifXY 6 2 if (isAnyPositive[1])
				setMetaPosteriorFN(metaData, posteriorFN[1])
			end

			@ifXY 7 2 if (isAnyPositive[1])
				setMetaAnteriorFP(metaData, anteriorFP[1])
			end
			@ifXY 8 2 if (isAnyPositive[1])
				setMetaAnteriorFN(metaData, anteriorFN[1])
			end

			@ifXY 9 2 if (isAnyPositive[1])
				setMetaTopFP(metaData, topFP[1])
			end
			@ifXY 10 2 if (isAnyPositive[1])
				setMetaTopFN(metaData, topFN[1])
			end

			@ifXY 11 2 if (isAnyPositive[1])
				setMetaBottomFP(metaData, bottomFP[1])
			end
			@ifXY 12 2 if (isAnyPositive[1])
				setMetaBottomFN(metaData, bottomFN[1])
			end


			@ifXY 1 3 if (isAnyPositive[1])
				setMetaDataTotalFpCount(metaData, shmemSum[1, 2], xOuter, yOuter, zOuter)
			end
			@ifXY 2 3 if (isAnyPositive[1])
				setMetaDataTotalFnCount(metaData, shmemSum[1, 1], xOuter, yOuter, zOuter)
			end
			#now in order to get total count  of fp and fn per block  we will do this subtraction              

			@ifXY 3 3 if (isAnyPositive[1])
				setMetaDataMainFpCount(metaData, shmemSum[1, 2] - leftFP[1] - rightFP[1] - posteriorFP[1] - anteriorFP[1] - topFP[1] - bottomFP[1], xOuter, yOuter, zOuter)
			end
			@ifXY 4 3 if (isAnyPositive[1])
				setMetaDataMainFnCount(metaData, shmemSum[1, 1] - leftFN[1] - rightFN[1] - posteriorFN[1] - anteriorFN[1] - topFN[1] - bottomFN[1], xOuter, yOuter, zOuter)
			end
