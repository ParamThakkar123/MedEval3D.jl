...
export bitDilatateUINtB, passOnesInner, @executeDataIter, @loadToshmemPaddings, @executeIterPadding, @loadToshmemPaddings, @validatePaddingInfo, @dilatateHelper, getDir, @validateData, @executeDataIterWithPadding, @loadMainValues,
	setNextBlockAsIsToBeActivated, @paddingProcessCombined, calculateLoopsIter, @processMaskData, @paddingIter, @processPadding
			...
			@unroll for bitIter in 1:32
				...
				if (resShemVal && !inSource)
					...
					if ((($refArr)[(($xMeta-1)*dataBdim[1]+threadIdxX()), ($yMeta-1)*dataBdim[2]+threadIdxY(), ($zMeta-1)*dataBdim[3]+bitIter]) == numberToLooFor)
						@addResult(metaData, $xMeta, $yMeta, $zMeta, resList, (($xMeta - 1) * dataBdim[1] + threadIdxX()), (($yMeta - 1) * dataBdim[2] + threadIdxY()), ($zMeta - 1) * dataBdim[3] + bitIter, getDir(shmemblockData, bitIter, dataBdim),
							$iterNumb, getIndexOfQueue(threadIdxX(), threadIdxY(), bitIter, dataBdim, (1 - $isGold)), metaDataDims, mainArrDims, $isGold)
					end
				end
			end
	...
	return esc(quote
		...
		for iterY in 0:inBlockLoopXZIterWithPadding
			if ((threadIdxY() + iterY * dataBdim[2]) <= dataBdim[1])
				for bitPos in 1:7
					@inbounds shmemPaddings[threadIdxX(), (threadIdxY()+iterY*dataBdim[2]), bitPos] = false
				end
			end
		end
		sync_threads()
		...
		if (locBool)
			@inbounds isAnythingInPadding[2] = true
		end
	...
	return esc(quote
		...
		if ($predicate)
			for bitPos in 1:32
				...
				@inbounds shmemPaddings[$padingVariedA, $padingVariedB, $paddingPos] = isBit1AtPos(shmemblockData[threadIdxX(), threadIdxY(), 1], bitPos)
				...
				if (shmemPaddings[$padingVariedA, $padingVariedB, $paddingPos])
					@inbounds isAnythingInPadding[$paddingPos] = true
				end
			end#for
...
# """
# Helper for bit operation
# """
# function passOnesInner(a::UInt32,b::UInt32,shmemblockData)
#     return esc(quote
#     if(a!=1 || b!=0)
#         c= @bitPassOnes a b
#         @inbounds shmemblockData[threadIdxX(),threadIdxY(),2] = c
#     end    
#     end)
# end
...
then 
"""
macro executeDataIter(mainArrDims, mainArr, refArr, xMeta, yMeta, zMeta, isGold, iterNumb)
	return esc(quote
		############## upload data
		@ifY 1 if (threadIdxX() < 15)
			areToBeValidated[threadIdxX()] = (metaData[($xMeta), ($yMeta), ($zMeta), (getIsToBeAnalyzedNumb()+threadIdxX())] == 1)
		end
		...
		@inbounds $mainArr[(($xMeta-1)*dataBdim[1]+threadIdxX()), (($yMeta-1)*dataBdim[2]+threadIdxY()), $zMeta] = @inbounds(shmemblockData[threadIdxX(), threadIdxY(), 2])
		...
		@ifXY 7 1 if (isMaskFull)
			metaData[$xMeta, $yMeta, $zMeta, getFullInSegmNumb()-$isGold] = 1
		end
...
macro executeIterPadding(mainArr, refArr, xMeta, yMeta, zMeta, isGold, iterNumb)
	return esc(quote
		...
		# @inbounds 
		@inbounds shmemblockData[threadIdxX(), threadIdxY(), 1] = @inbounds $mainArr[($xMeta-1)*dataBdim[1]+threadIdxX(), ($yMeta-1)*dataBdim[2]+threadIdxY(), $zMeta]
		...
		#top
		@validatePaddingInfo(shmemPaddings[threadIdxX(), threadIdxY(), 7], threadIdxX(), threadIdxY(), 1, $isGold,
			6, 7, $refArr, $xMeta, $yMeta, $zMeta, $iterNumb
		)
		#bottom
		@validatePaddingInfo(shmemPaddings[threadIdxX(), threadIdxY(), 2], threadIdxX(), threadIdxY(), dataBdim[3], $isGold,
			5, 2, $refArr, $xMeta, $yMeta, $zMeta, $iterNumb
		)
		sync_threads()
		...
		@unroll for bitPos in 1:32
			...
			@ifY 1 if (threadIdxX() <= dataBdim[2])
				@validatePaddingInfo(shmemPaddings[bitPos, threadIdxX(), 3], 1, threadIdxX(), bitPos, $isGold,
					2, 3, $refArr, $xMeta, $yMeta, $zMeta, $iterNumb
				)
			end
			#right
			@ifY 2 if (threadIdxX() <= dataBdim[2])
				@validatePaddingInfo(shmemPaddings[bitPos, threadIdxX(), 4], dataBdim[1], threadIdxX(), bitPos, $isGold,
					1, 4, $refArr, $xMeta, $yMeta, $zMeta, $iterNumb
				)
			end
			...
			@ifY 3 begin
				@validatePaddingInfo(
					shmemPaddings[threadIdxX(), bitPos, 5], threadIdxX(), dataBdim[2], bitPos, $isGold,
					3, 5, $refArr, $xMeta, $yMeta, $zMeta, $iterNumb
				)
			end
			#posterior
			@ifY 4 begin
				@validatePaddingInfo(shmemPaddings[threadIdxX(), bitPos, 6], threadIdxX(), 1, bitPos, $isGold,
					4, 6, $refArr, $xMeta, $yMeta, $zMeta, $iterNumb)
			end
		...
		@inbounds $mainArr[(($xMeta-1)*dataBdim[1]+threadIdxX()), (($yMeta-1)*dataBdim[2]+threadIdxY()), ($zMeta)] = @inbounds shmemblockData[threadIdxX(), threadIdxY(), 1]
	...
	return esc(
		quote
			...
			if ($shmemVal)
				...
				if ($refArr[(($xMeta-1)*dataBdim[1]+$xpos), (($yMeta-1)*dataBdim[2]+$ypos), (($zMeta-1)*dataBdim[3]+$zpos)]
					==
					numberToLooFor)
					# if(($xMeta)>3)
					#     CUDA.@cuprint "post reff  in validate bitPP $(bitPP) xPP $(xPP) shmm $(shmm) smhmemPadLay $(smhmemPadLay)\n"
					# end    
					@addResult(metaData, $xMeta, $yMeta, $zMeta, resList, (($xMeta - 1) * dataBdim[1] + $xpos), (($yMeta - 1) * dataBdim[2] + $ypos), (($zMeta - 1) * dataBdim[3] + $zpos), $dir, $iterNumb,
						getIndexOfQueue($xpos, $ypos, $zpos, dataBdim, (1 - $isGold)), metaDataDims, mainArrDims, $isGold)
...
macro loadToshmemPaddings(xMeta, yMeta, zMeta, xMetaChange, yMetaChange, zMetaChange, bitOfIntrest, shmemPaddingTargetNumb)
	return esc(
		quote
			...
			if (
				((($xMeta) + $xMetaChange) <= metaDataDims[1])
				&& (($xMeta) + $xMetaChange > 0)
				&& (($yMeta) + $yMetaChange > 0)
				...
				@unroll for iterY in 0:inBlockLoopXZIterWithPadding
					if ((threadIdxY() + iterY * dataBdim[2]) <= dataBdim[1])
						...
						# end                   
						@inbounds shmemPaddings[threadIdxX(), (threadIdxY()+iterY*dataBdim[2]), $shmemPaddingTargetNumb] = booll
					end
				end#for    
...
function setNextBlockAsIsToBeActivated(isToBeActivated::Bool, xMetaChange, yMetaChange, zMetaChange, xMeta, yMeta, zMeta, isGold, metaData, metaDataDims)
	if (isToBeActivated && (xMeta) + xMetaChange <= metaDataDims[1] && (xMeta) + xMetaChange > 0 && (yMeta) + yMetaChange > 0 && (yMeta) + yMetaChange <= metaDataDims[2] && (zMeta) + zMetaChange > 0 && (zMeta) + zMetaChange <= metaDataDims[3])
		@inbounds metaData[(xMeta+1)+xMetaChange, (yMeta+1)+yMetaChange, (zMeta+1)+zMetaChange, getIsToBeActivatedInSegmNumb()-isGold] = 1
	end
end
