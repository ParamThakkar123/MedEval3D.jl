...
macro uploadMinMaxesToShmem()
	return esc(quote
		@ifXY 1 1 if (anyPositive)
			minX[1] = min(minX[1], xMeta + 1)
		end
		@ifXY 1 2 if (anyPositive)
			maxX[1] = max(maxX[1], xMeta + 1)
		end
		@ifXY 2 1 if (anyPositive)
			minY[1] = min(minY[1], yMeta + 1)
		end
		@ifXY 2 2 if (anyPositive)
			maxY[1] = max(maxY[1], yMeta + 1)
		end
		@ifXY 3 1 if (anyPositive)
			minZ[1] = min(minZ[1], zMeta + 1)
		end
		@ifXY 3 2 if (anyPositive)
			maxZ[1] = max(maxZ[1], zMeta + 1)
		end
	end)
		...
		quote
			...
			HFUtils.@iter3dOuter(metaDataDims, loopMeta, metaDataLength,
				begin
					...
					@iterDataBlockZdeepest(mainArrDims, dataBdim, inBlockLoopX, inBlockLoopY, inBlockLoopZ, xMeta, yMeta, zMeta, begin
...
function getSmallForBoolKernel()
	return (CuArray([Float32(1110.0)]), CuArray([Float32(0.0)]), CuArray([Float32(1110.0)]), CuArray([Float32(0.0)]), CuArray([Float32(1110.0)]), CuArray([Float32(0.0)]), CuArray([UInt32(0)]), CuArray([UInt32(0)]))
end
