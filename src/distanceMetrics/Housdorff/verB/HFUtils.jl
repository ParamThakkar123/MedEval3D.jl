...
macro iterDataBlock(mainArrDims, dataBlockDims, loopXdim, loopYdim, loopZdim, xMeta, yMeta, zMeta, ex)
	mainExp = generalizedItermultiDim(; arrDims = mainArrDims, loopXdim, loopYdim, loopZdim
		...
		, xCheck = :(((xdim * blockDimX()) + threadIdxX()) <= $dataBlockDims[1] && x <= $mainArrDims[1]), yCheck = :(((ydim * blockDimY()) + threadIdxY()) <= $dataBlockDims[2] && y <= $mainArrDims[2]),
		zCheck = :((zdim + 1) <= $dataBlockDims[3] && z <= $mainArrDims[3]), zOffset = :($zMeta * (($dataBlockDims[3]))), zAdd = :(zdim + 1), yOffset = :(ydim * blockDimY() + $yMeta * $dataBlockDims[2]), yAdd = :(threadIdxY()),
		xOffset = :((xdim * blockDimX()) + $xMeta * $dataBlockDims[1]), xAdd = :(threadIdxX()), isFullBoundaryCheckX = true, isFullBoundaryCheckY = true, isFullBoundaryCheckZ = true,
		additionalActionBeforeX = :(xpos = xdim * blockDimX() + threadIdxX(); ypos = (ydim * blockDimY()) + threadIdxY(); zpos = (zdim + 1)), ex = ex)
	return esc(:($mainExp))
end
...
macro iterDataBlockZdeepest(mainArrDims, dataBlockDims, loopXdim, loopYdim, loopZdim, xMeta, yMeta, zMeta, ex, additionalActionAfterX)
	mainExp = generalizedItermultiDimZdeepest(; arrDims = mainArrDims, loopXdim, loopYdim, loopZdim
		# ,xCheck = :((xMeta* $dataBlockDims[1]+x)<=$mainArrDims[1] )
		# ,yCheck = :((yMeta* $dataBlockDims[2]+y)<=$mainArrDims[2])
		# ,zCheck = :( (zMeta* $dataBlockDims[3]+z)<=$mainArrDims[3])
		, xCheck = :(((xdim * blockDimX()) + threadIdxX()) <= $dataBlockDims[1] && x <= $mainArrDims[1]), yCheck = :(((ydim * blockDimY()) + threadIdxY()) <= $dataBlockDims[2] && y <= $mainArrDims[2]),
		zCheck = :((zdim + 1) <= $dataBlockDims[3] && z <= $mainArrDims[3]), zOffset = :($zMeta * (($dataBlockDims[3]))), zAdd = :(zdim + 1), yOffset = :(ydim * blockDimY() + $yMeta * $dataBlockDims[2]), yAdd = :(threadIdxY()),
		xOffset = :((xdim * blockDimX()) + $xMeta * $dataBlockDims[1]), xAdd = :(threadIdxX()), isFullBoundaryCheckX = true, isFullBoundaryCheckY = true, isFullBoundaryCheckZ = true,
		additionalActionBeforeX = :(xpos = xdim * blockDimX() + threadIdxX(); ypos = (ydim * blockDimY()) + threadIdxY()), additionalActionBeforeZ = :(zpos = (zdim + 1)), additionalActionAfterX = additionalActionAfterX, ex = ex)
	return esc(:($mainExp))
end
