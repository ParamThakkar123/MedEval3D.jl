...
macro analyzeMetadataFirstPass()
	return esc(quote
		...
		@metaDataWarpIter(metaDataDims, loopWarpMeta, metaDataLength, begin
			...
			@ifY 1 if (shmemPaddings[threadIdxX()+32] && isInRange)
				begin
					@appendToLocalWorkQueue((xMeta + 1), (yMeta + 1), (zMeta + 1), 0)
				end
			end
			@ifY 2 if (shmemPaddings[threadIdxX()] && isInRange)
				begin
			...
			@exOnWarp 3 if ((shmemSum[threadIdxX(), 15]) > 0 && isInRange)
				setBlockasCurrentlyActiveInSegm(metaData, xMeta + 1, yMeta + 1, zMeta + 1)
			end
			@exOnWarp 4 if ((shmemSum[threadIdxX(), 16]) > 0 && isInRange)
				setBlockasCurrentlyActiveInGold(metaData, xMeta + 1, yMeta + 1, zMeta + 1)
			end
			...
			@exOnWarp 5 begin
				if ((shmemSum[threadIdxX(), 15]) > 0 && isInRange)
					@unroll for i in 0:6
						#set fp
			...
			@exOnWarp 6 begin
				if ((shmemSum[threadIdxX(), 16]) > 0 && isInRange)
					@unroll for i in 0:6
						#set fn
...
macro checkIsActiveOrFullOr()
	return esc(quote
		@exOnWarp 30 if (isInRange)
			shmemPaddings[(threadIdxX())] = @accMeta(getFullInGoldNumb())
		end#  isBlockFulliInGold(metaData, xMeta,yMeta+1,zMeta+1)
		@exOnWarp 31 if (isInRange)
			shmemPaddings[(threadIdxX())+33] = @accMeta(getIsToBeActivatedInGoldNumb())
		end # isBlockToBeActivatediInGold(metaData, xMeta,yMeta+1,zMeta+1)
		@exOnWarp 32 if (isInRange)
			shmemPaddings[(threadIdxX())+33*2] = @accMeta(getActiveGoldNumb())
		end # isBlockCurrentlyActiveiInGold(metaData, xMeta,yMeta+1,zMeta+1)

		@exOnWarp 33 if (isInRange)
			shmemPaddings[(threadIdxX())+33*3] = @accMeta(getFullInSegmNumb())
		end # isBlockFullInSegm(metaData, xMeta,yMeta+1,zMeta+1)
		@exOnWarp 34 if (isInRange)
			shmemPaddings[(threadIdxX())+33*4] = @accMeta(getIsToBeActivatedInSegmNumb())
		end # isBlockToBeActivatedInSegm(metaData, xMeta,yMeta+1,zMeta+1)
		@exOnWarp 35 if (isInRange)
			shmemPaddings[(threadIdxX())+33*5] = @accMeta(getActiveSegmNumb())
		end # isBlockCurrentlyActiveInSegm(metaData, xMeta,yMeta+1,zMeta+1)
	end)#quote
