...
macro clearBeforeNextDilatation(clearIterResShmemLoop, clearIterSourceShmemLoop, resShmemTotalLength, sourceShmemTotalLength, dataBdim)
	return esc(quote
		...
		@ifY 1 if (threadIdxX() < 15)
			areToBeValidated[threadIdxX()] = false
		end
		@ifY 2 if (threadIdxX() < 8)
			isAnythingInPadding[threadIdxX()] = false
		end
	end)#quote
	...
	return esc(
		quote
			...
			@iterateOverWorkQueue(
				begin
					@executeIterPadding(dilatationArrs[shmemSum[shmemIndex*4+4]+1], referenceArrs[shmemSum[shmemIndex*4+4]+1], shmemSum[shmemIndex*4+1], shmemSum[shmemIndex*4+2], shmemSum[shmemIndex*4+3], shmemSum[shmemIndex*4+4], iterationNumberShmem[1]#iterNumb
					)
			...
			@iterateOverWorkQueue(
				begin
					@executeDataIter(mainArrDims, dilatationArrs[shmemSum[shmemIndex*4+4]+1], referenceArrs[shmemSum[shmemIndex*4+4]+1], shmemSum[shmemIndex*4+1], shmemSum[shmemIndex*4+2], shmemSum[shmemIndex*4+3], shmemSum[shmemIndex*4+4],
...
macro mainLoopKernel()
	return esc(quote
		...
		for i in 1:11
			@ifXY 1 1 if (blockIdxX() == 1)
				CUDA.@cuprint " goldToBeDilatated[1] $(goldToBeDilatated[1])  segmToBeDilatated[1] $(segmToBeDilatated[1]) workCounterBiggerThan0[1] $(workCounterBiggerThan0[1]) iterationNumberShmem[1] $(iterationNumberShmem[1]) \n"
			end
			@mainLoop()
		end
...
function getBigGPUForHousedorffAfterBoolKernel(metaData, minxRes, maxxRes, minyRes, maxyRes, minzRes, maxzRes, fn, fp, reducedGoldA, reducedSegmA, dataBdim)
	...
	goldArr = reducedGoldA[Int64(minxRes[1] * dataBdim[1]):Int64(maxxRes[1] * dataBdim[1]), Int64(minyRes[1] * dataBdim[2]):Int64(maxyRes[1] * dataBdim[2]), Int64(minzRes[1]):Int64(maxzRes[1]),
	]
	segmArr = reducedSegmA[Int64(minxRes[1] * dataBdim[1]):Int64(maxxRes[1] * dataBdim[1]), Int64(minyRes[1] * dataBdim[2]):Int64(maxyRes[1] * dataBdim[2]), Int64(minzRes[1]):Int64(maxzRes[1]),
	]
	...
	return (newMeta, goldArr, segmArr, paddingStore, resList, workQueue, workQueueCounter,
	)
end
end#MainLoopKernel